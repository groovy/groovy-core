...
A method is defined with the `def` keyword or with a return type.
...
Like constructors, normal methods can also be called with named arguments. They need to receive a map of parameters. In the method body, the values can be accessed as in normal maps (`map.key`).
...
Default arguments make parameters optional. In the case the argument is not supplied, the method assumes a default value.
